...
## Wire format
The Plexus relay is an append-only bus carrying `{name, text}`. A board message is `PIGEONv2:` + a JSON object in the `text` field. `name` must be on the relay roster (Roger, Catalyst, Pontus, Aria).
