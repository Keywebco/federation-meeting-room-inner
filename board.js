(function () {
  ...
  function setRelayStatus(live) {
    ...
    var bs = document.getElementById('relay-build-state');
    if (bs) { bs.className = 'val ' + (live ? 'live' : 'checking'); bs.textContent = live ? 'live ✓' : 'unreachable'; }
  }
  ...
  function render() {
    ...
    Object.keys(cols).forEach(function (key) {
      ...
      el.innerHTML = cols[key].map(renderThread).join('') || '<p class="dim">— empty —</p>';
  ...
  function statusChange(threadId, status, by) {
    ...
    if (status === 'sealed') {
      var loc = prompt('Write-back location (file/path the decision will be written to):', '');
      writeBack = { status: 'pending', location: loc || '', by: 'Catalyst (builder)' };
      note = 'sealed — write-back assigned to the builder';
    }
    postToRelay(by, { v: 2, kind: 'status', thread: threadId, status: status, by: by, ts: stamp(), note: note, write_back: writeBack });
  }
  ...
  document.getElementById('compose-form').addEventListener('submit', function (e) {
    ...
    var obj = {
      ...
      status_log: [{ ts: stamp(), by: document.getElementById('f-from').value, status: 'posted', note: isReply ? 'reply' : 'new thread' }]
    };
    btn.disabled = true;
